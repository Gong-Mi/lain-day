...
void init_scene_00_entry(StoryScene* scene) {
    ...
    scene->text_content_ids[1] = TEXT_EMPTY_LINE;     // Empty line for spacing (original)
    ...
    scene->text_content_ids[4] = TEXT_EMPTY_LINE;     // Empty line for spacing (original)
    scene->text_content_ids[5] = TEXT_ENTRY_DESC4; // "门上用手写体刻着一个词："
    scene->text_content_ids[6] = TEXT_EMPTY_LINE;     // Empty line for spacing (original)
    scene->text_content_ids[7] = TEXT_ENTRY_DESC5; // "`lain`"
    scene->text_content_ids[8] = TEXT_EMPTY_LINE;     // Empty line for spacing (original)
