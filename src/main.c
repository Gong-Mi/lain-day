...
// Helper function to filter out ANSI escape codes from the input buffer
static void filter_ansi_escape_codes(char* buffer, size_t len) {
    char* src = buffer;
    char* dst = buffer;

    while (*src != '\0' && (size_t)(src - buffer) < len) {
        if (*src == '\x1b') { // Found an escape character
            src++; // Skip '['
            if (*src == '[') {
                src++; // Skip parameters (digits, ';') until a letter
                while (*src != '\0' && !isalpha((unsigned char)*src) && (size_t)(src - buffer) < len) {
                    src++;
                }
                if (*src != '\0') { // Skip the final letter (e.g., 'A' for Up, 'D' for Left)
                    src++;
                }
            }
            // If it was just ESC without [, or an unrecognized sequence, we still skip the ESC
        } else {
            *dst++ = *src++; // Copy non-escape character
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    if (is_test_mode) {
        ...
        // In test mode, we create a temporary character.json from the hardcoded data.
        char temp_char_path[MAX_PATH_LENGTH];
        snprintf(temp_char_path, MAX_PATH_LENGTH, "%s/test_char.json", paths.session_root_dir); // Use session_root_dir for temp storage
        mkdir(paths.session_root_dir, 0755); // Ensure temp dir exists
        ...
        // We will not delete the temp file here; it will persist until the next clean build or manual deletion.
...
void get_next_input(char* buffer, int buffer_size, int argc, char* argv[], int* arg_index) {
    if (argc > 1 && *arg_index < argc) {
        ...
        if (fgets(buffer, buffer_size, stdin) != NULL) {
            ...
            filter_ansi_escape_codes(buffer, buffer_size);
        }
    }
}
